...
class StrategySelector:
    ...
    def _suggest_capital(self, signal: StrategySignal, ev: float, risk: float, total_capital: float | None) -> float:
        ...
        cap_limit = self.config.absolute_trade_capital_usdc
        if total_capital <= SMALL_ACCOUNT_THRESHOLD_USDC:
            cap_limit = min(cap_limit, max(SMALL_ACCOUNT_MIN_TRADE_USDC, total_capital * 0.375))
        return min(cap_limit, max(0.0, capital))
