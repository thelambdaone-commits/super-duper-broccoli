...
DB_DIR = Path(os.getenv("DATA_PATH", Path(__file__).resolve().parents[2] / "runtime" / "data"))
SNAPSHOT_DB_PATH = str(DB_DIR / "snapshots.duckdb")
    ...
    def capture(self, category: str, component: str, data: dict, tags: Optional[list[str]] = None):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Snapshot encryption unavailable for [%s:%s], storing plain JSON: %s",
                    category,
                    component,
                    exc,
                )
