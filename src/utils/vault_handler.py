...
def _load_env_file() -> None:
    """Charge le fichier .env depuis la racine ou secrets/ si présent."""
...
def _iter_encrypted_wallet_paths() -> list[Path]:
    primary = Path(POLYMARKET_WALLET_PATH)
    candidates = [primary]

    legacy_path = Path("data") / "polymarket.wallet.enc"
    runtime_path = Path("runtime") / "data" / "polymarket.wallet.enc"
