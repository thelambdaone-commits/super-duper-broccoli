...
async def _precheck_available_collateral(freqai: Any, side: str, price: float, size: float) -> Optional[dict]:
    if not freqai:
        return None
    if str(side).upper() not in {"BUY", "YES", "LONG"}:
        return None
    snapshot_fn = getattr(freqai, "get_available_collateral_usdc", None)
    ...
    if required_notional > available_balance:
        ...
        if price > 0 and available_balance >= minimum_notional:
            adjusted_size = math.floor(available_balance / float(price))
            adjusted_notional = float(adjusted_size) * float(price)
            if adjusted_size > 0 and adjusted_notional >= minimum_notional:
                return {
                    "status": "ADJUSTED",
                    "reason": (
                        f"Reducing order to available collateral: requested {required_notional:.2f} USDC, "
                        f"available {available_balance:.2f} USDC "
                        f"(total={total_balance:.2f}, locked_in_open_buys={locked_balance:.2f})"
                    ),
                    "adjusted_size": float(adjusted_size),
                    "adjusted_notional_usdc": adjusted_notional,
        ...
        return {
            "status": "SKIPPED",
            "reason": (
                f"Insufficient available collateral: required {required_notional:.2f} USDC, "
                f"available {available_balance:.2f} USDC "
                f"(total={total_balance:.2f}, locked_in_open_buys={locked_balance:.2f})"
            ),
            "required_notional_usdc": required_notional,
            "available_balance_usdc": available_balance,
            "total_balance_usdc": total_balance,
            "locked_balance_usdc": locked_balance,
        }
...
async def _execute_guarded(
        ...
        if str(mode).upper() != "REPLAY" and freqai and hasattr(freqai, "client") and hasattr(freqai.client, "get_order_book"):
            ...
            if isinstance(bids, (list, tuple)) and isinstance(asks, (list, tuple)) and bids and asks:
                ...
                if best_bid > 0 and best_ask > 0:
                    ...
                    # LOBSTAR V2: More flexible slippage gate
                    if price_diff > threshold:
                        # If we are within 2x the threshold, we log a warning but proceed if FORCE_PROD is on
                        # otherwise we reject to be safe.
                        if price_diff <= threshold * 2.0 and os.getenv("FORCE_PROD", "").lower() == "true":
                            logger.warning(
                                f"⚠️ [SLIPPAGE WARNING] High deviation ({price_diff:.2%}), but proceeding due to FORCE_PROD. "
                                f"Mid: {mid_price:.4f}, Signal: {price:.4f}"
                            )
                        else:
                            logger.warning(
    ...
    collateral_block = await _precheck_available_collateral(freqai, side, normalized_price, final_size)
    if collateral_block:
        if collateral_block["status"] == "ADJUSTED":
            adjusted_size = float(collateral_block["adjusted_size"])
            logger.warning(
                "💸 [COLLATERAL GATE] %s | size %.2f -> %.2f",
                collateral_block["reason"],
                final_size,
                adjusted_size,
            )
