...
def _is_local_sizing_rejection(error: object) -> bool:
    message = str(error)
    return (
        "[Sizing]" in message
        or "minimum Polymarket" in message
        or "min_order_size" in message
    )
...
class PassiveExecutor:
    ...
    def _is_strict_maker_only(self) -> bool:
        ...
        if self.ledger:
            try:
                ...
                logger.warning("Failed to read safety flags from database: %s", exc)
        return False
    ...
    async def _maker_first(
        ...
        try:
            ...
            self._record_latency(time.time() - start_t)
            if _is_local_sizing_rejection(e):
                logger.info("Maker sizing rejected locally for %s: %s", ticker, e)
        ...
        while time.time() < deadline:
            ...
            try:
                ...
                if not status or not isinstance(status, dict):
                    logger.debug(f"Invalid status response for {order_id}: {status}")
                    continue
    ...
    async def _taker(
        ...
        if self.ledger:
            try:
                ...
                logger.warning("Failed to read execution mode from database, defaulting to PAPER: %s", exc)
