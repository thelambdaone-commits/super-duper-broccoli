...
def _build_allocation_from_fills(fills_df: pd.DataFrame) -> pd.DataFrame:
    ...
    side_sign = pd.to_numeric(
        fills["order_side"].map({"BUY": 1.0, "SELL": -1.0}), errors="coerce"
    ).fillna(0.0)
...
def create_monthly_returns_heatmap(
    ...
    fig.update_layout(
        title=title, xaxis_title="Month", yaxis_title="Year", template="plotly_white", height=400
    )
...
def _render_pnl(
    ...
    **kwargs: Any,
    ...
    if not account_equity.empty:
        ...
        fig.update_yaxes(
            title_text=f"PnL ({currency})" if currency is not None else "PnL", row=row, col=col
        )
...
def _render_monthly_returns(
    fig: go.Figure, row: int, col: int, returns: pd.Series, **kwargs: Any
) -> None:
    """
...
def _render_yearly_returns(
    ...
    **kwargs: Any,
    ...
    fig.add_trace(
        go.Bar(
            x=yearly.index.year, y=yearly.to_numpy(), marker={"color": colors}, showlegend=False
        ),
        row=row,
        col=col,
    )
...
_register_tearsheet_chart(
    "bars_with_fills", "scatter", "Bars with Order Fills", _render_bars_with_fills
)
