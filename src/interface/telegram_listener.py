...
class TelegramListener:
    ...
    async def _augment_btc_tracking_text(self, interval: str, base_text: str) -> str:
        ...
        open_positions = paper_positions + live_positions
        
        closed = 0
        for pos in open_positions:
            is_live_pos = "position_id" in pos and str(pos["position_id"]).startswith("live-")
            ...
            if stop_loss_cents > 0 and adverse_move >= stop_loss_cents:
                ...
                if is_live_pos:
                    self._ledger.close_position(pos["position_id"], exit_price=live_price, pnl=pnl, exit_reason="stop_loss")
                else:
                    self._ledger.close_paper_position(pos["position_id"], exit_price=live_price, pnl=pnl, is_win=pnl > 0)
                closed += 1

        refreshed_paper = [p for p in self._ledger.get_paper_positions(status="OPEN") if p.get("ticker") == f"BTC_{interval}"]
        refreshed_live = [p for p in self._ledger.get_open_positions() if p.get("ticker") == f"BTC_{interval}"]
        refreshed_positions = refreshed_paper + refreshed_live
        
        tracking_lines = [
            "",
            "<b>Live Tracking</b>",
            f"• Open live/paper positions: <code>{len(refreshed_live)} / {len(refreshed_paper)}</code>",
    ...
    async def _cmd_wallet_cockpit(self, update: Update, _context) -> None:
        ...
        # Delegate to orchestrator or specific handler
        await self._handle_wallet_cockpit(update)

    async def _handle_wallet_cockpit(self, update: Update) -> None:
        msg = getattr(update, "effective_message", None) or getattr(update, "message", None)
        chat_id = getattr(msg, "chat_id", None)
    ...
    async def _cmd_regime(self, update: Update, _context) -> None:
        if not self._hmm:
            ...
            return
        try:
            from strategies.hmm_filter import REGIME_LABELS
            import numpy as np
    ...
    async def _cmd_positions(self, update: Update, _context) -> None:
        ...
        try:
            ...
            logger.error(f"Error in /p command: {e}", exc_info=True)
            await self.reply_to(f"❌ Erreur: {e}", update)
        ...
        if query.data.startswith("wallet_select:"):
            ...
            if len(parts) == 3:
                ...
                try:
                    ...
                    mode = self._ledger.get_execution_mode()
                    is_live = (mode == "PROD")
                    prefix = "⚡ LIVE" if is_live else "🧾 PAPER"
                    
                    if is_live:
                        import uuid, time
                        position_id = f"live-BTC_{interval}-{direction.upper()}-{int(time.time())}-{uuid.uuid4().hex[:6]}"
                    ...
                    if order.get("error"):
                        await query.answer(f"{prefix} trade rejected.", show_alert=True)
                        return
                    self._ledger.set_position_stop_loss_cents(order["position_id"], 0.05)
                    await query.answer(f"{prefix} {direction.upper()} BTC {interval} placé.")
                    await query.message.reply_text(
                        (
                            f"<b>{prefix} BTC {interval.upper()}</b>\n"
                            "───────────────────\n"
                    ...
                    logger.exception("BTC callback failed")
                    await query.answer(f"Trade failed: {exc}", show_alert=True)
            else:
                await query.answer()
            return
            ...
            if len(parts) == 3:
                ...
                if not self._ledger:
                    ...
                    return
                ticker = f"BTC_{interval}"
                paper_positions = [p for p in self._ledger.get_paper_positions(status="OPEN") if p.get("ticker") == ticker]
                live_positions = [p for p in self._ledger.get_open_positions() if p.get("ticker") == ticker]
                all_pos = paper_positions + live_positions
                for pos in all_pos:
                    self._ledger.set_position_stop_loss_cents(pos["position_id"], stop_loss_cents)
                await query.answer(
                    f"SL {stop_loss_cents:.2f} appliqué à {len(all_pos)} position(s)." if all_pos else "Aucune position BTC ouverte."
                )
            else:
                await query.answer()
            return
            ...
            live_positions = self._ledger.get_open_positions()
            ticker = f"BTC_{interval}"
            
            to_close_paper = [pos for pos in open_positions if pos.get("ticker") == ticker]
            for pos in to_close_paper:
                self._ledger.close_paper_position(pos["position_id"])
                
            to_close_live = [pos for pos in live_positions if pos.get("ticker") == ticker]
            for pos in to_close_live:
                self._ledger.close_position(pos["position_id"], exit_reason="user_cancelled")
                
            total_closed = len(to_close_paper) + len(to_close_live)
            
            await query.answer(
                f"{total_closed} trade(s) BTC {interval} annulé(s)." if total_closed else f"Aucun trade BTC {interval} ouvert."
            )
            if total_closed:
                await query.message.reply_text(
                    (
                        f"🛑 <b>BTC {interval.upper()} Cancel</b>\n"
                        "───────────────────\n"
                        f"• Closed positions: <code>{total_closed}</code> (Live: {len(to_close_live)})\n"
    ...
    async def start(self) -> None:
        ...
        while self._running:
            ...
            try:
                ...
                # SENSITIVE COMMANDS DISABLED FOR PRODUCTION SECURITY
